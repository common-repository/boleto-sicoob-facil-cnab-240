...
class BoletoSicoobFacilCnab_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'boleto-sicoob-facil-cnab'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'boleto-sicoob-facil-cnab'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'boleto-sicoob-facil-cnab'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'boleto-sicoob-facil-cnab'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'boleto-sicoob-facil-cnab');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'boleto-sicoob-facil-cnab'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'boleto-sicoob-facil-cnab');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'boleto-sicoob-facil-cnab'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'boleto-sicoob-facil-cnab') ?>"/>
                </p>
            </form>
			
			
			<form action="" method="post">
    <input type="hidden" name="reset_pages_boleto_sicoob_facil_cnab" value="yes">
    <table cellspacing="1" cellpadding="2">
      <tbody>
        <tr> 
          <td> <p>
              <?php _e('<h3>Reset Pages: If the new version did not work then click the Reset Pages button.</h3>','boleto-sicoob-facil-cnab') ?>
            </p>
            <?php _e('The pages will be re-created. You`ll need to change the menus on your site so your customers can find them.','boleto-sicoob-facil-cnab') ?>
            <p class="submit"> 
              <input name="submit" type="submit" class="button-primary"

                           value="<?php _e('Reset Pages','boleto-sicoob-facil-cnab') ?>"/>
            </p></td>
        </tr>
    </table>
  </form>
 
   <p><?php if(isset($_POST['reset_pages_boleto_sicoob_facil_cnab'])){

	include( BOLETO_SICOOB_FACIL_CNAB_DIR .'contents/boleto/reset/all-pages.php');

	include( BOLETO_SICOOB_FACIL_CNAB_DIR .'contents/boleto/create/all-pages.php');
...
?></p> 
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'boleto-sicoob-facil-cnab');
            case 'false':
                return __('false', 'boleto-sicoob-facil-cnab');

            case 'Administrator':
                return __('Administrator', 'boleto-sicoob-facil-cnab');
            case 'Editor':
                return __('Editor', 'boleto-sicoob-facil-cnab');
            case 'Author':
                return __('Author', 'boleto-sicoob-facil-cnab');
            case 'Contributor':
                return __('Contributor', 'boleto-sicoob-facil-cnab');
            case 'Subscriber':
                return __('Subscriber', 'boleto-sicoob-facil-cnab');
            case 'Anyone':
                return __('Anyone', 'boleto-sicoob-facil-cnab');
        }
        return $optionValue;
    }
